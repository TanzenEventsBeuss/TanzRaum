...
export default function App() {
    ...
    weekDates.forEach((d, di) => {
      ...
      getEvents(ds).forEach(ev => {
        ...
        const sub = ev.type==="window"
          ? `${ev.start}–${ev.end} · ${room?.name||""} · Zum Buchen ziehen`
          : `${ev.start}–${ev.end}${ev.instructor?" · "+ev.instructor:""}${ev.customer?" · "+ev.customer:""}${room&&selRoom==="all"?" · "+room.name:""}`;
        b.innerHTML = `<div class="sb-title">${ev.type==="window"?"⬛ "+label:label}</div><div class="sb-sub">${sub}</div>`;

        if(isKunde && ev.type==="window"){
          // Drag-to-select within window
          b.onmousedown = (e) => {
            e.preventDefault();
            const rect = b.getBoundingClientRect();
            const relY = e.clientY - rect.top;
            const startRow = Math.floor(relY / ROW_H);
            const absTop = top + startRow*ROW_H;
            dragState.current = {di, ds, startRow, currentRow:startRow, windowSlot:ev, leftPx, widthPx, headerH, blockTop:top, blockHeight:height};
            setDragPreview({top:absTop, height:ROW_H, left:leftPx+2, width:widthPx-4});
          };
  ...
  // ── Drag handlers for window booking ─────────────────────────────────────
  useEffect(() => {
    const onMove = (e) => {
      ...
      const relY = e.clientY - rect.top - ds.headerH - ds.blockTop;
      const row = Math.max(0, Math.min(Math.floor(relY / ROW_H), Math.floor(ds.blockHeight/ROW_H)-1));
      ds.currentRow = row;
      const minRow = Math.min(ds.startRow, row);
      const maxRow = Math.max(ds.startRow, row);
      const selTop = ds.headerH + ds.blockTop + minRow*ROW_H;
      const selH   = (maxRow - minRow + 1) * ROW_H;
    ...
    const onUp = () => {
      ...
      if(durRows >= 2) { // min 30 min = 2 rows
        ...
        // Open booking modal with selected time range
        setBookingModal({
          ...ds.windowSlot,
          _dragStart: selStart,
          _dragEnd:   selEnd,
    ...
    <div className="wrap">
      ...
      <div className="legend">
        ...
        <div style={{marginLeft:"auto",fontSize:11,color:"var(--muted)"}}>
          {isKunde
            ? "Grüne Slots anklicken · Im Zeitfenster Wunschzeit per Drag auswählen"
            : "Klick auf leere Zelle = Slot anlegen"}
        </div>
      </div>

      <div className="cal-outer">
        <div className={`cal-container${isKunde?" kunde-mode":""}`} style={{position:"relative"}}>
          ...
          {dragPreview && (
            <div className="drag-select" style={{
              ...
              <div style={{padding:"2px 6px",fontSize:11,fontWeight:600,color:"var(--green)"}}>
                {Math.floor(dragPreview.height/ROW_H)*15} min
              </div>
            </div>
          )}
        </div>
  ...
  const SlotModalForm = () => {
    ...
    const save = async()=>{
      ...
      const savedId=await dbSaveSlot({id:isEdit?s.id:null,room,title,type,date,start,end,customer:cust,note,price:parseFloat(price)||0});
      if(!savedId){showToast("Fehler beim Speichern.","error");return;}
