...
:root{--bg:#0a0a0f;--surface:#13131a;--surface2:#1c1c26;--border:#1e1e2a;--accent:#ff3c6e;--accent2:#ff8c42;--gold:#ffd166;--text:#f0f0f5;--muted:#666677;--green:#06d6a0;--red:#ef233c;--blue:#4ecdc4;--purple:#9b5de5;--r:12px}
...
.kunde-mode .slot-block.free{animation:slotPulse 2.4s ease-in-out infinite}
...
@media(max-width:700px){.stat-grid{grid-template-columns:1fr 1fr}.admin-layout{grid-template-columns:1fr}.admin-sidebar{display:none}.form-row,.form-row3{grid-template-columns:1fr}.nav-tabs{display:none}}
`;
...
export default function App() {
  ...
  const renderBlocks = useCallback(() => {
    ...
    weekDates.forEach((d, di) => {
      ...
      getEvents(ds).forEach(ev => {
        ...
        const top=headerH+(cs/15)*ROW_H, height=Math.max(ROW_H,(ce-cs)/15*ROW_H)-2;
        ...
        const sub = `${ev.start}–${ev.end}${ev.instructor?" · "+ev.instructor:""}${ev.customer?" · "+ev.customer:""}${room&&selRoom==="all"?" · "+room.name:""}`;
        b.innerHTML = `<div class="sb-title">${label}</div><div class="sb-sub">${sub}</div>`;
        b.onclick = () => {
          if(ev._kind==="course"){if(isAdmin)setCourseModal(ev);return;}
          if(isKunde){if(ev.type==="free")setBookingModal(ev);return;}
          setSlotModal(ev);
        };
        grid.appendChild(b);
      });
    });
  }, [weekDates, filteredRoomIds, filterType, isKunde, isAdmin, selRoom, rooms, slots, courses, weekStart]);
  ...
  // ── Render helpers ────────────────────────────────────────────────────────
  const SL = {free:"Frei",booked:"Gebucht",blocked:"Gesperrt",pending:"Anfrage",course:"Kurs"};
  const SC = {free:"var(--green)",booked:"var(--blue)",blocked:"var(--red)",pending:"var(--gold)",course:"var(--purple)"};

  const fmt = d => d.toLocaleDateString("de-DE",{day:"2-digit",month:"long"});
  const weekLabel = `${fmt(weekDates[0])} – ${fmt(weekDates[6])} ${weekDates[0].getFullYear()}`;
  ...
  const CalendarGrid = () => {
    ...
    for(let row=0;row<totalRows;row++){
      ...
      weekDates.forEach((_,di)=>{
        ...
        const cellDate = dateStr(weekDates[di]);
        gridRows.push(
          <div key={`c${row}-${di}`}
  ...
  const renderKalender = () => (
    <div className="wrap">
      ...
      <div className="legend">
        ...
        <div className="legend-item"><span className="ldot" style={{background:"var(--green)"}}></span>Frei</div>
        ...
        <div style={{marginLeft:"auto",fontSize:11,color:"var(--muted)"}}>
          {isKunde?"Grüne Slots anklicken → Kontaktformular":"Klick auf leere Zelle = Slot anlegen"}
        </div>
      </div>

      <div className="cal-outer">
        <div className={`cal-container${isKunde?" kunde-mode":""}`}>
  ...
  const SlotModalForm = () => {
    ...
    const save = async()=>{
      ...
      await dbSaveSlot({id:isEdit?s.id:null,room,title,type,date,start,end,customer:cust,note,price:parseFloat(price)||0});
      await reload("slots"); setSlotModal(null); showToast(isEdit?"Slot aktualisiert.":"Slot angelegt.","success");
    };
    const TYPES=["free","booked","blocked","pending"];
    const TLABELS={free:"Frei",booked:"Gebucht",blocked:"Gesperrt",pending:"Anfrage"};
    return (
  ...
  const CourseModalForm = () => {
    ...
    const save = async()=>{
      ...
      await dbSaveCourse({id:isEdit?c.id:null,name,room,start,end,recurType,days,validFrom,validTo,onceDate,instructor,note,price:hasPrice?(parseFloat(price)||0):0});
      await reload("courses"); setCourseModal(null); showToast(isEdit?"Kurs aktualisiert.":"Kurs angelegt.","success");
    };
    return (
  ...
  const BookingModalForm = () => {
    ...
    return (
      <div className="overlay open" onClick={e=>{if(e.target===e.currentTarget)setBookingModal(null);}}>
        <div className="modal">
          <div className="modal-title">Slot anfragen</div>
          <div style={{background:"var(--surface2)",border:"1px solid rgba(6,214,160,.25)",borderRadius:10,padding:"14px 16px",marginBottom:22}}>
            <div style={{fontSize:10,textTransform:"uppercase",letterSpacing:1,color:"var(--muted)",marginBottom:6}}>Ausgewählter Slot</div>
            <div style={{fontSize:16,fontWeight:600}}>{s?.title}</div>
            <div style={{fontSize:13,color:"var(--muted)",marginTop:3}}>{df} · {s?.start}–{s?.end} · {room?.name||"?"}{loc?` · ${loc.name}`:""}</div>
            {s?.price>0&&<div style={{fontSize:13,color:"var(--gold)",marginTop:5,fontWeight:600}}>{s.price.toFixed(2)} €</div>}
          </div>
          {done
    ...
    <>
      ...
      <div className="nav">
        ...
        <div className="nav-tabs">
          {[{id:"kalender",label:"Kalender"},{id:"liste",label:"Buchungen"},{id:"raeume",label:"Säle"},{id:"uebersicht",label:"Übersicht",staffOnly:true},{id:"admin",label:"Admin",adminOnly:true}].map(tab=>{
