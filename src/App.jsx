...
export default function App() {
  ...
  const [windowPickerModal, setWindowPickerModal] = useState(null);
    ...
    visibleDates.forEach((d, di) => {
      ...
      getEvents(ds).forEach(ev => {
        ...
        if(isKunde && ev.type==="window"){
          b.onclick = () => setWindowPickerModal(ev);
          b.style.cursor = "pointer";
          b.style.touchAction = "auto";
          // Show helpful label
          b.innerHTML = `<div class="sb-title">⏱ ${label}</div><div class="sb-sub">${ev.start}–${ev.end} · Antippen zum Buchen</div>`;
        } else {
          b.onclick = () => {
    ...
    <div className="wrap">
      ...
      <div className="legend">
        ...
        <div style={{marginLeft:"auto",fontSize:11,color:"var(--muted)"}}>
          {isKunde
            ? "Slots antippen zum Buchen · Zeitfenster antippen → Uhrzeit wählen"
  ...
  // ── Window Picker Modal ───────────────────────────────────────────────────
  const WindowPickerModal = () => {
    const ev = windowPickerModal;
    const room = rooms.find(r=>r.id===ev?.room);
    const loc  = room ? locations.find(l=>l.id===room.location) : null;
    const df   = ev ? new Date(ev.date+"T12:00").toLocaleDateString("de-DE",{weekday:"long",day:"2-digit",month:"long",year:"numeric"}) : "";

    // Build 15-min time options within the window
    const winStart = t2m(ev?.start||"00:00");
    const winEnd   = t2m(ev?.end||"23:59");
    const opts = [];
    for(let m=winStart; m<winEnd; m+=15) opts.push(m2t(m));

    const [selStart, setSelStart] = useState(opts[0]||ev?.start||"");
    const [selEnd,   setSelEnd]   = useState(() => {
      const def = winStart + 60;
      return m2t(Math.min(def, winEnd));
    });
    const [err, setErr] = useState("");

    const endOpts = opts.filter(o => t2m(o) > t2m(selStart)).concat([ev?.end||""]).filter((v,i,a)=>a.indexOf(v)===i);

    const dur = t2m(selEnd) - t2m(selStart);
    const durLabel = dur >= 60
      ? `${Math.floor(dur/60)}h${dur%60>0?" "+dur%60+"min":""}` 
      : `${dur} min`;

    const proceed = () => {
      if(t2m(selEnd) - t2m(selStart) < 30){
        setErr("Bitte mindestens 30 Minuten wählen.");
        return;
      }
      setWindowPickerModal(null);
      setBookingModal({
        ...ev,
        _dragStart: selStart,
        _dragEnd:   selEnd,
    ...
    return (
      <div className="overlay open" onClick={e=>{if(e.target===e.currentTarget)setWindowPickerModal(null);}}>
        <div className="modal" style={{maxWidth:440}}>
          {/* Header */}
          <div style={{textAlign:"center",marginBottom:20,paddingBottom:16,borderBottom:"1px solid var(--border)"}}>
            <div style={{fontSize:36,marginBottom:8}}>⏱</div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,letterSpacing:3,color:"var(--accent2)"}}>Zeitraum wählen</div>
            <div style={{fontSize:13,color:"var(--muted)",marginTop:4}}>{room?.name||""}{loc?` · ${loc.name}`:""}</div>
            <div style={{fontSize:12,color:"var(--muted)",marginTop:2}}>{df}</div>
          </div>

          {/* Available window info */}
          <div style={{background:"rgba(255,140,66,.08)",border:"1px solid rgba(255,140,66,.25)",borderRadius:10,padding:"10px 14px",marginBottom:18,display:"flex",alignItems:"center",gap:10}}>
            <div style={{fontSize:20}}>📆</div>
            <div>
              <div style={{fontSize:11,color:"var(--accent2)",fontWeight:600,textTransform:"uppercase",letterSpacing:.5}}>Verfügbares Zeitfenster</div>
              <div style={{fontSize:15,fontWeight:600,marginTop:2}}>{ev?.start} – {ev?.end} Uhr</div>
            </div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
            <div>
              <div className="form-label" style={{marginBottom:6,fontSize:12}}>🕐 Startzeit</div>
              <select className="inp" style={{fontSize:15,padding:"12px 14px",fontWeight:600,color:"var(--text)"}}
                value={selStart}
                onChange={e=>{
                  setSelStart(e.target.value);
                  setErr("");
                  if(t2m(selEnd)<=t2m(e.target.value))
                    setSelEnd(m2t(Math.min(t2m(e.target.value)+60,winEnd)));
                }}>
                {opts.map(o=><option key={o} value={o}>{o} Uhr</option>)}
              </select>
            </div>
            <div>
              <div className="form-label" style={{marginBottom:6,fontSize:12}}>🕑 Endzeit</div>
              <select className="inp" style={{fontSize:15,padding:"12px 14px",fontWeight:600,color:"var(--text)"}}
                value={selEnd}
                onChange={e=>{setSelEnd(e.target.value);setErr("");}}>
                {opts.filter(o=>t2m(o)>t2m(selStart)).map(o=><option key={o} value={o}>{o} Uhr</option>)}
                {winEnd > t2m(opts[opts.length-1]||"0:00") && <option value={ev?.end}>{ev?.end} Uhr</option>}
              </select>
            </div>
          </div>
          ...
          {/* Duration display */}
          {!err && dur >= 30 && (
            <div style={{textAlign:"center",padding:"12px 0",marginBottom:12,borderRadius:10,background:"rgba(6,214,160,.08)",border:"1px solid rgba(6,214,160,.2)"}}>
              <div style={{fontSize:11,color:"var(--muted)",textTransform:"uppercase",letterSpacing:.5,marginBottom:2}}>Gewählte Dauer</div>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:32,letterSpacing:2,color:"var(--green)",lineHeight:1}}>{durLabel}</div>
              <div style={{fontSize:12,color:"var(--muted)",marginTop:2}}>{selStart} – {selEnd} Uhr</div>
            </div>
          )}

          {err && <div style={{color:"var(--red)",fontSize:13,textAlign:"center",marginBottom:12,padding:"10px",background:"rgba(239,35,60,.08)",borderRadius:8,border:"1px solid rgba(239,35,60,.2)"}}>{err}</div>}

          <div className="modal-actions">
            <button className="btn btn-ghost" onClick={()=>setWindowPickerModal(null)}>Abbrechen</button>
            <button className="btn btn-primary" style={{flex:2,justifyContent:"center",fontSize:15}} onClick={proceed}>
              Weiter zur Buchung →
            </button>
          </div>
        </div>
  ...
  return (
    <>
      ...
      {windowPickerModal&& <WindowPickerModal/>}
